% RF_ITER_ALL
% Compute Iterative time domain RF deconvolution 
% based on algorithm on Ligorria & Ammon (BSSA,89-5,1999)
%
% Input:  ZRT data that come from the script rot_all.m or rot_all_BH.m
% Output: RF traces
% Notes:
% Signals are in SAC format.
% Function requested: rf_iter
%
% EXAMPLE:
% 1 earthquake happened the 6 April 2010 at 22:54:06 (M=5.3) 
...
% Output:
% 100406T225406.AIGLE.RFi.R.SAC
% 100406T225406.AIGLE.RFi.T.SAC
% 
% Original program by György Hétenyi
% Edited by Leonardo Colavitti
% 14 July       2016 for Iter5s_2Hz
% 23 September  2016 for Iter10s_1Hz
% -------------------------------------------------------------------------

% Add useful directories
addpath /Users/lcolavit/Documents/MATLAB/Tools;
addpath /Users/lcolavit/Documents/MATLAB/Data_Download;

% CHANGE HERE the input base folder
% dir0='/Users/lcolavit/Database/Data_Swiss/';
dir0='/Users/lcolavit/Database/ALL_DATA/';
dir1=[dir0 'ZNE_file/HH/'];
dir2=[dir0 'RF/Iter10s_1Hz/HH/']; % Write output here!
event_file=[dir1 'listevents_ALL'];
tr_list_file='/listZNERT';
% -------------------------------------------------------------------------
% PARAMETERS DEFINITION
% -------------------------------------------------------------------------
lo=1/10;        % Lower cut-off frequency [Hz] 
hi=1;           % High cut-off frequency  [Hz]
...
for iev=1:nev;
   ...
   if length(tnames{1})~=0;
      ...
      % Display check number of station to be processed 
      % (Here I have only 1 station)
...
% -------------------------------------------------------------------------
     % Write out, here goes the name of the variable tnames!
            ...
            end % End fit computation 
         end % End fit real processing   
      end % End loop on stations   
