% PLOT_RMSCOLOR
% Plot color/bw rms-maps between variables VS1, VS2, H
% H--VS1 , H--VS2 , H--VpVs
% VS1--VS2, VS1--VpVs, VS2--VpVs
% Use after read_nad
%
...
ldens=10;
isvpvs=1;
colmap=flipud(gray); %flipud(gray);
close
...
for i=1:nblay-1
   ...
   D(1).rms=zeros(length(D(1).xx),length(D(1).yy));
   ...
   D(2).rms=zeros(length(D(2).xx),length(D(2).yy));
   ...
   D(3).rms=zeros(length(D(3).xx),length(D(3).yy));
   ...
   D(4).rms=zeros(length(D(4).xx),length(D(4).yy));
   ...
   D(5).rms=zeros(length(D(5).xx),length(D(5).yy));
   ...
   D(6).rms=zeros(length(D(6).xx),length(D(6).yy));
   ...
   for j=1:lrms
      ...
      D(1).rms(ind)=D(1).rms(ind)+rmsv(j)*nind;
      ...
      D(2).rms(ind)=D(2).rms(ind)+rmsv(j)*nind;
      ...
      if isvpvs==1
         ...
         D(3).rms(ind)=D(3).rms(ind)+rmsv(j)*nind;
      end   
      ...
      D(4).rms(ind)=D(4).rms(ind)+rmsv(j)*nind;
      ...
      if isvpvs==1
         ...
         D(5).rms(ind)=D(5).rms(ind)+rmsv(j)*nind;
      end   
      if isvpvs==1
         ...
         D(6).rms(ind)=D(6).rms(ind)+rmsv(j)*nind;
      end
   end
   
   if isvpvs==1,subplot(2,3,1);else,subplot(2,2,1);end
   
   pcolor(D(1).xx,D(1).yy,D(1).rms'./D(1).dens')
   shading flat
   caxis('auto')%caxis([6 9])
   xlabel('H');ylabel('Vs1')
   
   if isvpvs==1,subplot(2,3,2);else,subplot(2,2,2);end

   pcolor(D(2).xx,D(2).yy,D(2).rms'./D(2).dens')
   shading flat
   caxis('auto')%caxis([6 9])
   xlabel('H');ylabel('Vs2')

   if isvpvs==1,subplot(2,3,3)
      pcolor(D(3).xx,D(3).yy,D(3).rms'./D(3).dens')
      shading flat
      caxis('auto')%caxis([6 9])
   ...
   pcolor(D(4).xx,D(4).yy,D(4).rms'./D(4).dens')
   shading flat
   caxis('auto')%caxis([6 9])
   xlabel('Vs1');ylabel('Vs2')
   
   if isvpvs==1,subplot(2,3,5)
      pcolor(D(5).xx,D(5).yy,D(5).rms'./D(5).dens')
      shading flat
      caxis('auto')%caxis([6 9])
   ...
   if isvpvs==1,subplot(2,3,6)
      pcolor(D(6).xx,D(6).yy,D(6).rms'./D(6).dens')
      shading flat
      caxis('auto')%caxis([6 9])
